...
vAppr = tk.Tk()
vAppr.title('View Appointments')
vAppr.configure(background='sea green')

screen_width = vAppr.winfo_screenwidth()
screen_height = vAppr.winfo_screenheight() 

x = (screen_width / 2) - (app_width / 2)
y = (screen_height / 2) - (app_height / 2)

vAppr.geometry(f'{app_width}x{app_height}+{int(x)}+{int(y)}')

#submenu labels and positioning
menu = Menu(vAppr)
vAppr.config(menu=menu)
