...
eAppdn = tk.Tk()
eAppdn.title('Edit Appointment')
eAppdn.configure(background='sea green')

screen_width = eAppdn.winfo_screenwidth()
screen_height = eAppdn.winfo_screenheight() 

x = (screen_width / 2) - (app_width / 2)
y = (screen_height / 2) - (app_height / 2)

eAppdn.geometry(f'{app_width}x{app_height}+{int(x)}+{int(y)}')

#submenu labels and positioning
menu = Menu(eAppdn)
eAppdn.config(menu=menu)
