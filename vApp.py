...
vApp = tk.Tk()
vApp.title('View Appointments')
vApp.configure(background='sea green')

screen_width = vApp.winfo_screenwidth()
screen_height = vApp.winfo_screenheight() 

x = (screen_width / 2) - (app_width / 2)
y = (screen_height / 2) - (app_height / 2)

vApp.geometry(f'{app_width}x{app_height}+{int(x)}+{int(y)}')

#submenu labels and positioning
menu = Menu(vApp)
vApp.config(menu=menu)
