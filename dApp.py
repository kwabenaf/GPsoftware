...
dApp = tk.Tk()
dApp.title('Cancel Appointment')
dApp.configure(background='sea green')

screen_width = dApp.winfo_screenwidth()
screen_height = dApp.winfo_screenheight() 

x = (screen_width / 2) - (app_width / 2)
y = (screen_height / 2) - (app_height / 2)

dApp.geometry(f'{app_width}x{app_height}+{int(x)}+{int(y)}')

#submenu labels and positioning
menu = Menu(dApp)
dApp.config(menu=menu)
