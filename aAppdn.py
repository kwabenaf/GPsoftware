...
aAppdn = tk.Tk()
aAppdn.title('Book Appointment')
aAppdn.configure(background='sea green')

#centers the app on the screen
screen_width = aAppdn.winfo_screenwidth()
screen_height = aAppdn.winfo_screenheight() 

x = (screen_width / 2) - (app_width / 2)
y = (screen_height / 2) - (app_height / 2)

aAppdn.geometry(f'{app_width}x{app_height}+{int(x)}+{int(y)}')

#submenu labels and positioning
menu = Menu(aAppdn)
aAppdn.config(menu=menu)
